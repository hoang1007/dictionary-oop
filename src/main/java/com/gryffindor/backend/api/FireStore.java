...
public class FireStore {
    ...
    try {
      GoogleCredentials credentials =
          GoogleCredentials.fromStream(
              DictionaryApplication.INSTANCE.config.getGoogleServiceStream());
  ...
  public static void add(Word word) throws InterruptedException, ExecutionException {
    ApiFuture<WriteResult> future =
        database.collection("dictionary").document(word.getWordTarget()).set(word);
   ...
   * @return trả về từ muốn tìm nếu có trong database trả về null nếu không tìm thấy
  ...
  public static Word find(String wordTarget)
    ...
    ApiFuture<DocumentSnapshot> future =
        database.collection("dictionary").document(wordTarget).get();
  ...
  public static void deleteTranslation(Word word, Translation translation)
    ...
    System.out.println(
        docRef
            .update("translations", FieldValue.arrayRemove(translation))
  ...
  public static void updateTranslation(
      Word word, Translation oldTrans, @NonNull Translation newTrans)
      throws InterruptedException, ExecutionException, TimeoutException {
    DocumentReference docRef = database.collection("dictionary").document(word.getWordTarget());
    WriteBatch batch = database.batch();
  ...
  public static void addTranslation(Word word, Translation translation)
    ...
    System.out.println(
        docRef
            .update("translations", FieldValue.arrayUnion(translation))
            .get(TIMEOUT, TimeUnit.MINUTES)
            .getUpdateTime());
  }
}
