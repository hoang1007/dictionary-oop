...
public class GoogleTranslator {
   ...
   */
  public static String translate(String wordTarget, Language from, Language to) throws IOException {
    String urlString =
        DictionaryApplication.INSTANCE.config.getGoogleApiUrl()
            + "?q="
            + URLEncoder.encode(wordTarget, "UTF-8")
            + "&target="
            + to.toShortString()
            + "&from="
            + from.toShortString();
