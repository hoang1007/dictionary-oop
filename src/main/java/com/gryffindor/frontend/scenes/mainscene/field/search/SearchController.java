...
public class SearchController implements IController {
            ...
            (observable, oldValue, newValue) -> {
              ...
              if (oldValue.length() == 0 && newValue.length() == 1) {
                BinarySearch.setWordList(
                    DictionaryApplication.INSTANCE
                        .getDictionaryManagement()
                        .getDictionary()
                        .getWordList(newValue));
              }
  ...
  void actionOnClickImageSearch() {
    searchField
        .getImageSearchButton()
        .setOnAction(
            event -> {
              File img =
                  new FileChooserWindow("Choose image", "image")
                      .setExtensionFilter(
                          new ExtensionFilter("PNG", "*.png"), new ExtensionFilter("JPEG", "*.jpg"))
                      .getOpenFile();
    ...
    new Thread(
            () -> {
              ...
              if (status.equals(Status.ONLINE)) {
                ...
                word =
                    DictionaryApplication.INSTANCE.dictionaryManagement.searchWordOnline(
                        wordTarget);

              } else { // if OFFLINE
                System.out.println("Searching offline");
                word =
                    DictionaryApplication.INSTANCE.dictionaryManagement.dictionaryLookup(
                        wordTarget);
              }
